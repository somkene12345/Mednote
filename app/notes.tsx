...
export default function SearchPatientNotes() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Sort patients by most recent test start time
      resultPatients.sort((a, b) => {
        const lastTestA = a.sections[0]?.title ? new Date(a.sections[0].title.replace(" ", "T")) : new Date();
        const lastTestB = b.sections[0]?.title ? new Date(b.sections[0].title.replace(" ", "T")) : new Date();
        return lastTestB.getTime() - lastTestA.getTime();
      });
                  ...
                  <SectionList
                    ...
                    renderItem={({ item }) => (
                      <View style={styles.noteContainer}>
                        ...
                        {item.Comment && item.Comment.trim()To sort the patients by the most recent test start time and ensure the patient container takes the full width of its container when expanded, you can update the code as follows:

1. **Sort Patients by Test Start Time**: You can sort the `patients` array based on the most recent test start time.

2. **Expand Patient Container to Full Width**: You can modify the styles of the expanded patient container to take the full width of its parent.
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      ...
      try {
        ...
        // Sort the patients by the most recent test start time
        allPatients.sort((a, b) => {
          const aLastTest = new Date(a.sections[0]?.title.replace(" ", "T")).getTime();
          const bLastTest = new Date(b.sections[0]?.title.replace(" ", "T")).getTime();
          return bLastTest - aLastTest; // Sort descending (most recent first)
        });
